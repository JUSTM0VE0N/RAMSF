...
from data.data_msip import Load_Trainset
from data.data_hsip import Load_Trainset as Load_HSIP_Trainset
from data.data_mhif import Load_Trainset as Load_MHIF_Trainset
...
class BaseSolver:
    def __init__(self, cfg):
        ...
        if cfg['task'] == 'HSIP':
            self.training_set = Load_HSIP_Trainset(cfg['training_set'] + '/' + cfg['satellite4train'])
            self.training_data_loader = DataLoader(dataset=self.training_set, num_workers=self.num_worker, batch_size=cfg['batch_size'], shuffle=True,
                                                   pin_memory=True, drop_last=True)
            self.validate_set = Load_HSIP_Trainset(cfg['training_set'] + '/' + cfg['satellite4valid'])
            self.validate_data_loader = DataLoader(dataset=self.validate_set, num_workers=self.num_worker, batch_size=cfg['batch_size'], shuffle=True,
                                                   pin_memory=True, drop_last=True)

        if cfg['task'] == 'MHIF':
            self.training_set = Load_MHIF_Trainset(cfg['training_set'] + '/' + cfg['satellite4train'])
            self.training_data_loader = DataLoader(dataset=self.training_set, num_workers=self.num_worker, batch_size=cfg['batch_size'], shuffle=True,
                                                   pin_memory=True, drop_last=True)
            self.validate_set = Load_MHIF_Trainset(cfg['training_set'] + '/' + cfg['satellite4valid'])
            self.validate_data_loader = DataLoader(dataset=self.validate_set, num_workers=self.num_worker, batch_size=cfg['batch_size'], shuffle=True,
                                                   pin_memory=True, drop_last=True)
