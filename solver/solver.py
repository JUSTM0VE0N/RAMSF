...
class Solver(BaseSolver):
    def __init__(self, cfg):
        ...
        lib = importlib.import_module('models.' + cfg['task'] + '.' + net_name)
        net = lib.Network
        ...
        # save log
        self.writer = SummaryWriter('logs/' + self.cfg['task'] + '/' + self.cfg['model'] + '/' + str(self.log_name))
        save_net_config(cfg, self.log_name, self.model)
        save_net_py(cfg, self.log_name, net_name)
        save_yml(cfg, os.path.join('logs/' + self.cfg['task'] + '/' + cfg['model'] + '/' + str(self.log_name), 'config.yml'))
    ...
    def train(self):
        with tqdm(total=len(self.training_data_loader), miniters=1,
            ...
            for iteration, batch in enumerate(self.training_data_loader,1):
                ...
                out3 = self.model(pan.float(), lms.float(), ms.float())
                loss = self.loss(out3.float(), gt.float())
    ...
    def eval(self):
        with tqdm(total=len(self.validate_data_loader), miniters=1,
            ...
            for iteration, batch in enumerate(self.validate_data_loader, 1):
                ...
                t1.set_postfix_str(
                    '\033[32mPer batch validate loss: {:.7f}, ERGAS: {:.4f}, SAM: {:.4f}\033[0m'.format(
                        loss.item(), avg_ergas, avg_sam))
                t1.update()
            ...
            save_config(self.cfg, self.log_name,
                        'Validate Epoch {}: TLoss: {:.7f}, ERGAS: {:.4f}, SAM: {:.4f}'.format \
                            (self.epoch, self.records['VLoss'][-1], self.records['ERGAS'][-1], self.records['SAM'][-1]))
    ...
    def check_pretrained(self):
        checkpoint = os.path.join(self.cfg['pretrain']['pre_folder'], self.cfg['task'], self.cfg['satellite'], self.log_name, self.cfg['pretrain']['pre_source'])
    ...
    def save_checkpoint(self):
        ...
        self.ckpt_dir = self.cfg['checkpoint'] + '/' + self.cfg['task'] + '/' + self.cfg['model'] + '/' + self.cfg['satellite'] + '/' + self.log_name
