...
from data.data_msip import Load_RRTset, Load_FRTset
from data.data_hsip import Load_RRTset as Load_HSIP_Testset
from data.data_mhif import Load_RRTset as Load_MHIF_Testset
...
class Testsolver(BaseSolver):
    def __init__(self, cfg):
        ...
        lib = importlib.import_module(cfg['task'] + '.models.' + net_name)
        net = lib.Network
    ...
    def test4refer(self):
        ...
        for i, batch in enumerate(self.data_loader, 0):
            ...
            with torch.no_grad():
                ...
                out1, out2, out3, ms_u4 = self.model(ms.float(), pan.float())  # MMALPNet
            t1 = time.time()
            if self.cfg['test']['use_metric']:
                ...
                q2n = Q2n(ground_truth, predict_out)
                rase = RASE(ground_truth, predict_out)
                qave = QAVE(ground_truth, predict_out)
        ...
        if self.cfg['test']['use_metric']:
            ...
            avg_q2n = np.array(q2n_list).mean()
            avg_rase = np.array(rase_list).mean()
            avg_qave = np.array(qave_list).mean()
            print('===> Score on metrics')
            print('     AVG of Q2n: {:.4f}+-{:.4f}'.format(avg_q2n, np.array(q2n_list).std()))
            ...
            print('     AVG of RASE: {:.4f}+-{:.4f}'.format(avg_rase, np.array(rase_list).std()))
            ...
            print('     AVG of QAVE: {:.4f}+-{:.4f}'.format(avg_qave, np.array(qave_list).std()))
    ...
    def test4nonrefer(self):
        ...
        for i, batch in enumerate(self.data_loader, 0):
            ...
            if self.cfg['test']['use_metric']:
                ...
                d_lambda_k = D_lambda_K(predict_out, test_lms, 4, self.cfg['satellite'], 32)
    ...
    def save_img(self, img, img_name, mode):
        ...
        save_img = np.uint8(save_img*self.cfg['test']['bitdepth']).astype('uint8')
