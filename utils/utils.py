...
def save_config(cfg, time, log):
    open_type = 'a' if os.path.exists(get_path('./logs/' + cfg['task'] + '/' + cfg['model'] + '/' + str(time) + '/records.txt'))else 'w'
    log_file = open(get_path('./logs/' + cfg['task'] + '/' + cfg['model'] + '/' + str(time) + '/records.txt'), open_type)
    log_file.write(str(log) + '\n')

def save_net_config(cfg, time, log):
    Folder=get_path('./logs/' + cfg['task'] + '/' + cfg['model'] + '/' + str(time) + '/')
...
def save_net_py(cfg, time, py):
    shutil.copyfile(os.path.join('./models' + '/' + cfg['task'], py+'.py'), os.path.join('./logs/' + cfg['task'] + '/' + cfg['model'] + '/' + str(time), py+'.py'))
